...
namespace Algorist.EventSourcing.Util
{
    internal static class ReflectionHelper
    {
        ...
        public static Dictionary<Type, string> FindEventHandlerMethodsInAggregate<TAggregateKey>(Type aggregateType)
        {
            if (AggregateEventHandlerCache.ContainsKey(aggregateType) == false)
            {
                var eventHandlers = new ConcurrentDictionary<Type, string>();

                var typeArgs = new [] { typeof(TAggregateKey) };
                var eventType = typeof(IEvent<>).MakeGenericType(typeArgs);

                var voidMethods = aggregateType.GetMethodsBySig(typeof(void), typeof(AggregateEventApplier), true, eventType).ToList();
                var asyncMethods = aggregateType.GetMethodsBySig(typeof(Task), typeof(AggregateEventApplier), true, eventType).ToList();
